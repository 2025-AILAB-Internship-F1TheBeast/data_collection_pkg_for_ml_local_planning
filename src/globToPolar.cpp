...
public:
    ...
    {
        // 파라미터 선언 및 값 가져오기
        std::string path_csv_file = this->declare_parameter<std::string>("path_csv_file", "");
        ...
        // CSV 파일 경로 유효성 검사
        if (path_csv_file.empty()) {
            RCLCPP_ERROR(this->get_logger(), "path_csv_file parameter is required but not set. Shutting down.");
            rclcpp::shutdown();
            return;
        }
